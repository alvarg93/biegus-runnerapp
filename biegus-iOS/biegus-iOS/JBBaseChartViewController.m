...
- (void)setTooltipVisible:(BOOL)tooltipVisible animated:(BOOL)animated atTouchPoint:(CGPoint)touchPoint
{
    ...
    dispatch_block_t adjustTooltipPosition = ^{
        ...
        if (chartView)
        {
            ...
            CGPoint origin = chartView.headerView.frame.origin;
            CGPoint converted = [chartView.headerView convertPoint:origin toView:nil];
            
            self.tooltipView.frame = CGRectMake(convertedTouchPoint.x - ceil(self.tooltipView.frame.size.width * 0.5), CGRectGetMaxY(chartView.headerView.frame)+converted.y-10, self.tooltipView.frame.size.width, self.tooltipView.frame.size.height);
